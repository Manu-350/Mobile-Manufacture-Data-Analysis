...
--Q10--END
with cte as (
select year(Date)[Year],sum(TotalPrice)[Total_price] from FACT_TRANSACTIONS
group by year(Date)
)
select *,concat(round((total_price- lag(Total_price,1) over(order by Year))*100/lag(Total_price,1) over(order by Year),2),'%')[YOY Change] from cte